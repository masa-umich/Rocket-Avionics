...
void compute_fallback_times(float altitude, float velocity, float accel,
    ...
    // time to apogee: v = v0 + a*t, at apogee v=0 => t = -v0/a
    float t_apogee = (-velocity)/accel;
    float h_apogee = altitude + (velocity * velocity) / (2.0f * -accel);
    *apogee_time += (uint32_t)(t_apogee * 1000);

    // time to 5km altitude: after apogee, estimate descent at ~6 m/s terminal velocity
    float t_5k = t_apogee + (h_apogee - 5000.0f) / 6.0f;
    if (h_apogee < 5000.0f)
        t_5k = t_apogee;
    *five_k_time += (uint32_t)(t_5k * 1000);

    // time to 1km altitude: after drogue deploy, estimate descent at ~5.5 m/s terminal velocity
    float t_1k = t_5k + (5000.0f - 1000.0f) / 5.5f;
    if (h_apogee < 1000.0f)
        t_1k = t_apogee;
