...
int readADCRaw(ADC_HandleTypeDef* hadc1, int channels, int* adcValues, int maxDelayPerChannel) {
	// This function relies on HAL switching to the next ADC rank, but this causes issues with RTOS threads
	// To fix this we make this a "critical task" so that it's not interrupted.
	// Additionally, "Sampling time" should be set no lower than 64.5 per channel if you haven't already
	taskENTER_CRITICAL();
...
//Gives the voltage and time-stamps of all ADCs (0-3.3V) in an array that contains all channels
int readADC(ADC_HandleTypeDef* hadc1, int channels, float* adcValues, int maxDelayPerChannel) {
    ...
    // Get the timestamp of when the readings were taken and put it as the last value in the array
    uint64_t timestamp = getTimestamp();
    // Needs to be a float because it's a float array, but the return value is uint64_t
