...
#define W25N04KV_SPI_TIMEOUT                      (uint8_t)  0x0A

// 1024 blocks with 64 pages each = 65536 pages
#define W25N04KV_PAGES_PER_BLOCK                  (uint16_t) 64
#define W25N04KV_NUM_BLOCKS                       (uint16_t) 4096

// Used for find_file_ptr()
#define W25N04KV_ERASED_BYTE                               (uint8_t) 0xFF

/* Commands */
// Summary of commands and usage on datasheet pg 23-25
#define W25N04KV_DEVICE_RESET                     (uint8_t) 0xFF
#define W25N04KV_READ_JEDEC_ID                    (uint8_t) 0x9F
#define W25N04KV_READ_STATUS_REGISTER             (uint8_t) 0x0F  // 0x05 also works
#define W25N04KV_WRITE_STATUS_REGISTER            (uint8_t) 0x1F  // 0x01 also works
#define W25N04KV_WRITE_ENABLE                     (uint8_t) 0x06
#define W25N04KV_WRITE_DISABLE                    (uint8_t) 0x04
#define W25N04KV_ERASE_BLOCK                      (uint8_t) 0xD8
#define W25N04KV_LOAD_PROGRAM_DATA                (uint8_t) 0x02
#define W25N04KV_PROGRAM_EXECUTE                  (uint8_t) 0x10
#define W25N04KV_PAGE_DATA_READ                   (uint8_t) 0x13
#define W25N04KV_READ_DATA                        (uint8_t) 0x03

/* Status Register addressses */
#define W25N04KV_SR1_PROTECTION_REG_ADR           (uint8_t) 0xA0  // Listed as 0xAx in the datasheet
#define W25N04KV_SR2_CONFIG_REG_ADR               (uint8_t) 0xB0  // Listed as 0xBx in the datasheet
#define W25N04KV_SR3_STATUS_REG_ADR               (uint8_t) 0xC0  // Listed as 0xBx in the datasheet

/* Status Register bits */
// Protection register - datasheet pg 15
#define W25N04KV_SR1_BLOCK_PROTECT_BP3            (uint8_t) 0x40  // 0b01000000
#define W25N04KV_SR1_BLOCK_PROTECT_BP2            (uint8_t) 0x20  // 0b00100000
#define W25N04KV_SR1_BLOCK_PROTECT_BP1            (uint8_t) 0x10  // 0b00010000
#define W25N04KV_SR1_BLOCK_PROTECT_BP0            (uint8_t) 0x08  // 0b00001000
#define W25N04KV_SR1_BLOCK_PROTECT_TB             (uint8_t) 0x04  // 0b00000100
#define W25N04KV_SR1_WRITE_PROTECT_ENABLE         (uint8_t) 0x02  // 0b00000010
#define W25N04KV_SR1_STATUS_REGISTER_PROTECT_SRP1 (uint8_t) 0x00  // 0b10000000
#define W25N04KV_SR1_STATUS_REGISTER_PROTECT_SRP0 (uint8_t) 0x01  // 0b00000001

// Configuration register - datasheet pg 17
#define W25N04KV_SR2_ONE_TIME_PROGRAM_LOCK        (uint8_t) 0x80  // 0b10000000
#define W25N04KV_SR2_ENTER_TOP_ACCESS_MODE        (uint8_t) 0x40  // 0b01000000
#define W25N04KV_SR2_STATUS_REGISTER_1_LOCK       (uint8_t) 0x20  // 0b00100000
#define W25N04KV_SR2_ECC_ENABLE                   (uint8_t) 0x10  // 0b00010000
#define W25N04KV_SR2_BUFFER_READ_MODE             (uint8_t) 0x08  // 0b00001000

// Status register - datasheet pg 19
#define W25N04KV_SR3_ECC_STATUS_BIT_1             (uint8_t) 0x20  // 0b00100000
#define W25N04KV_SR3_ECC_STATUS_BIT_0             (uint8_t) 0x10  // 0b00010000
#define W25N04KV_SR3_PROGRAM_FAILURE              (uint8_t) 0x08  // 0b00001000
#define W25N04KV_SR3_ERASE_FAILURE                (uint8_t) 0x04  // 0b00000100
#define W25N04KV_SR3_WRITE_ENABLE_LATCH           (uint8_t) 0x02  // 0b00000010
#define W25N04KV_SR3_OPERATION_IN_PROGRESS        (uint8_t) 0x01  // 0b00000001

// Flash internal delays - datasheet pg 59
// Used for firmware timeouts
#define W25N04KV_RESET_MAX_TIME_US                    500  // datasheet pg 26
#define W25N04KV_WRITE_STATUS_REGISTER_TIME_NS         50
#define W25N04KV_BLOCK_ERASE_MAX_TIME_MS                4
#define W25N04KV_READ_PAGE_DATA_ECC_ON_MAX_TIME_US     60
#define W25N04KV_READ_PAGE_DATA_ECC_OFF_MAX_TIME_US    25
#define W25N04KV_PAGE_PROGRAM_MAX_TIME_US             700
 ...
 */
#define W25N04KV_PACK_2_BYTES_TO_UINT16(bytes)		(uint16_t) ((((uint16_t) *(bytes)) << 8) + *((bytes)+1))
 ...
 */
static void spi_transmit(W25N04KV_Flash *flash, uint8_t *tx, uint16_t size) {

	//__disable_irq();
	HAL_GPIO_WritePin(flash->cs_base, flash->cs_pin, W25N04KV_CS_ACTIVE);  // Select chip
	// Transmit data and store the status code
	flash->last_HAL_status = HAL_SPI_Transmit(flash->SPI_bus, tx, size, W25N04KV_SPI_TIMEOUT);
 ...
 */
static void spi_transmit_receive(W25N04KV_Flash *flash, uint8_t *tx,
 ...
 */
static uint8_t read_status_register(W25N04KV_Flash *flash, uint8_t register_adr) {
	uint8_t tx[2] = {W25N04KV_READ_STATUS_REGISTER, register_adr};
	uint8_t rx[1];
 ...
 */
static uint8_t flash_is_busy(W25N04KV_Flash *flash) {
	uint8_t status_register = read_status_register(flash, W25N04KV_SR3_STATUS_REG_ADR);
	return status_register & W25N04KV_SR3_OPERATION_IN_PROGRESS;
}
 ...
 */
static void wait_for_operation(W25N04KV_Flash *flash, uint32_t timeout) {
	uint32_t count = 0;
 ...
 */
static void write_status_register(W25N04KV_Flash *flash, uint8_t register_adr,
		uint8_t register_write_val) {
	uint8_t tx[3] = {W25N04KV_WRITE_STATUS_REGISTER, register_adr, register_write_val};

	spi_transmit(flash, tx, 3);

	wait_for_operation(flash, W25N04KV_WRITE_STATUS_REGISTER_TIME_NS);
}
...
static void load_page(W25N04KV_Flash *flash, uint32_t page_num) {
	...
	wait_for_operation(flash, W25N04KV_READ_PAGE_DATA_ECC_ON_MAX_TIME_US * 1000);  // Wait for the page to load
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void unlock_flash(W25N04KV_Flash *flash) {
	// Read the current contents of the protection register
	uint8_t protection_register = read_status_register(flash, W25N04KV_SR1_PROTECTION_REG_ADR);

	// Remove the block protect bits, and only keep the 3 non-protect bits, if they're already enabled.
	uint8_t unlocked_protection_register = protection_register & ~(W25N04KV_SR1_BLOCK_PROTECT_BP3
			| W25N04KV_SR1_BLOCK_PROTECT_BP2
			| W25N04KV_SR1_BLOCK_PROTECT_BP1
			| W25N04KV_SR1_BLOCK_PROTECT_BP0
			| W25N04KV_SR1_BLOCK_PROTECT_TB);

	// Write the new value of the status register block protect bits off
	write_status_register(flash, W25N04KV_SR1_PROTECTION_REG_ADR, unlocked_protection_register);
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void lock_flash(W25N04KV_Flash *flash) {
	// Read the current contents of the protection register
	uint8_t protection_register = read_status_register(flash, W25N04KV_SR1_PROTECTION_REG_ADR);
	...
	uint8_t locked_protection_register = protection_register & ~(W25N04KV_SR1_BLOCK_PROTECT_BP3
			| W25N04KV_SR1_BLOCK_PROTECT_BP2
			| W25N04KV_SR1_BLOCK_PROTECT_BP1
			| W25N04KV_SR1_BLOCK_PROTECT_BP0
			| W25N04KV_SR1_BLOCK_PROTECT_TB);
	...
	locked_protection_register = protection_register
			| (W25N04KV_SR1_BLOCK_PROTECT_BP3 | W25N04KV_SR1_BLOCK_PROTECT_BP2);

	write_status_register(flash, W25N04KV_SR1_PROTECTION_REG_ADR, locked_protection_register);
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void enable_write(W25N04KV_Flash *flash) {
	uint8_t tx[1] = { W25N04KV_WRITE_ENABLE };
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void disable_write(W25N04KV_Flash *flash) {
	uint8_t tx[1] = { W25N04KV_WRITE_DISABLE };
	spi_transmit(flash, tx, 1);
}
 ...
 */
static void write_page_to_buffer(W25N04KV_Flash *flash, uint8_t *data,
		uint16_t num_bytes, uint16_t column_adr) {

	uint8_t column_adr_8bit_array[2] = W25N04KV_UNPACK_UINT16_TO_2_BYTES(column_adr);
	uint8_t tx1[3] = {W25N04KV_LOAD_PROGRAM_DATA, column_adr_8bit_array[0], column_adr_8bit_array[1]};
	...
	if (num_bytes > W25N04KV_BYTES_PER_PAGE)
		num_bytes = W25N04KV_BYTES_PER_PAGE;

	// Not using spi_transmit() because I didn't want to mess with combining the tx arrays
	//__disable_irq();
	HAL_GPIO_WritePin(flash->cs_base, flash->cs_pin, W25N04KV_CS_ACTIVE);
	flash->last_HAL_status = HAL_SPI_Transmit(flash->SPI_bus, tx1, 3, W25N04KV_SPI_TIMEOUT);
	flash->last_HAL_status = HAL_SPI_Transmit(flash->SPI_bus, data, num_bytes, W25N04KV_SPI_TIMEOUT);
	HAL_GPIO_WritePin(flash->cs_base, flash->cs_pin, W25N04KV_CS_INACTIVE);
	//__enable_irq();
}
 ...
 */
static uint8_t get_write_failure_status(W25N04KV_Flash *flash) {
	// If it can't read from flash, it will automatically return a write failure
	if (ping_flash(flash)) {
		uint8_t status_register = read_status_register(flash, W25N04KV_SR3_STATUS_REG_ADR);
		flash->last_write_failure_status = status_register & W25N04KV_SR3_PROGRAM_FAILURE;
	}
	else {
		flash->last_write_failure_status = W25N04KV_SR3_PROGRAM_FAILURE;
	}
 ...
 */
static uint8_t get_erase_failure_status(W25N04KV_Flash *flash) {
	// If it can't read from flash, it will automatically return an erase failure
	if (ping_flash(flash)) {
		uint8_t status_register = read_status_register(flash, W25N04KV_SR3_STATUS_REG_ADR);
		flash->last_erase_failure_status = status_register & W25N04KV_SR3_ERASE_FAILURE;
	}
	else {
		flash->last_erase_failure_status = W25N04KV_SR3_ERASE_FAILURE;
	}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void get_ECC_status(W25N04KV_Flash *flash) {

	// If it can read from flash properly, check the ECC bits as normal
	if (ping_flash(flash)) {
		uint8_t status_register, ECC1, ECC0;

		status_register = read_status_register(flash, W25N04KV_SR3_STATUS_REG_ADR);
		ECC1 = status_register & W25N04KV_SR3_ECC_STATUS_BIT_1;
		ECC0 = status_register & W25N04KV_SR3_ECC_STATUS_BIT_0;
 ...
 */
static void read_flash_buffer(W25N04KV_Flash *flash, uint8_t *buffer,
		uint16_t num_bytes, uint16_t column_adr) {
 ...
 */
static void read_bytes_from_page(W25N04KV_Flash *flash, uint8_t *buffer, uint16_t num_bytes,
		uint32_t page_adr, uint16_t column_adr) {
 ...
 */
static void write_bytes_to_page(W25N04KV_Flash *flash, uint8_t *data, uint16_t num_bytes,
		uint32_t page_adr, uint16_t column_adr) {
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void enable_ECC(W25N04KV_Flash *flash) {
	uint8_t config_reg_read = read_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR);
	uint8_t ECC_enabled_register = config_reg_read | W25N04KV_SR2_ECC_ENABLE;	 // OR: turn bit on

	if (ECC_enabled_register != config_reg_read)
		write_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR, ECC_enabled_register);
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void disable_ECC(W25N04KV_Flash *flash) {
	uint8_t config_reg_read = read_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR);
	uint8_t ECC_disabled_register = config_reg_read & ~W25N04KV_SR2_ECC_ENABLE;	 // Remove bit

	if (ECC_disabled_register != config_reg_read)
		write_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR, ECC_disabled_register);
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void enable_buffer_mode(W25N04KV_Flash *flash) {
	uint8_t config_reg_read = read_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR);
	uint8_t buffer_enabled_register = config_reg_read | W25N04KV_SR2_BUFFER_READ_MODE;	// OR: turn bit on
	if (buffer_enabled_register != config_reg_read)
		write_status_register(flash, W25N04KV_SR2_CONFIG_REG_ADR, buffer_enabled_register);
}
 ...
 * @param flash      <W25N01GV_Flash*>    Struct used to store flash pins and addresses
 */
static void find_write_ptr(W25N04KV_Flash *flash) {
	...
	uint32_t max = W25N04KV_NUM_PAGES;  // exclusive
	uint32_t cur_search_page;
	...
	if (read_buffer[2047] != 0xFF) {  // aforementioned edge case
		if (flash->current_page == W25N04KV_NUM_PAGES-1) {  // no room left in flash
...
void init_flash(W25N04KV_Flash *flash, SPI_HandleTypeDef *SPI_bus_in,
...
uint8_t ping_flash(W25N04KV_Flash *flash) {

	uint8_t tx[2] = {W25N04KV_READ_JEDEC_ID, 0};	// Second byte is unused
	uint8_t rx[3];

	spi_transmit_receive(flash, tx, 2, rx, 3);
	uint8_t manufacturer_ID = rx[0];
	uint16_t device_ID = W25N04KV_PACK_2_BYTES_TO_UINT16(rx+1);
...
uint8_t reset_flash(W25N04KV_Flash *flash) {
	...
	uint8_t tx[1] = { W25N04KV_DEVICE_RESET };
	spi_transmit(flash, tx, 1);

	// Wait for it to reset
	wait_for_operation(flash, W25N04KV_RESET_MAX_TIME_US * 1000);
 ...
 *
 */
static uint16_t write_to_flash_contiguous(W25N04KV_Flash *flash, uint8_t *data, uint32_t num_bytes) {
	...
	*/
	...
	while (write_counter < num_bytes) {
		...
		if (num_bytes_to_write_on_page > W25N04KV_BYTES_PER_PAGE - flash->next_free_column)
			num_bytes_to_write_on_page = W25N04KV_BYTES_PER_PAGE - flash->next_free_column;
		...
		if (flash->next_free_column + num_bytes_to_write_on_page < W25N04KV_BYTES_PER_PAGE)
		...
		else if (flash->current_page == W25N04KV_NUM_PAGES-1)
			flash->next_free_column = W25N04KV_BYTES_PER_PAGE;
...
uint16_t write_to_flash(W25N04KV_Flash *flash, uint8_t *data, uint32_t num_bytes) {
	...
	if (flash->write_buffer_size > 0 && flash->write_buffer_size + num_bytes >= W25N04KV_SECTOR_SIZE) {
		// Copy data into write_buffer until it's full,
		uint16_t num_bytes_to_copy = W25N04KV_SECTOR_SIZE - flash->write_buffer_size;
		for (uint16_t i = 0; i < num_bytes_to_copy; ++i) {
			flash->write_buffer[flash->write_buffer_size + i] = data[i];
		}
		flash->write_buffer_size = W25N04KV_SECTOR_SIZE;
		buffer_full = 1;
	...
	else if (flash->write_buffer_size + num_bytes < W25N04KV_SECTOR_SIZE) {
	...
	uint32_t new_data_size = (num_bytes / W25N04KV_SECTOR_SIZE) * W25N04KV_SECTOR_SIZE;
	uint32_t end_size = num_bytes % W25N04KV_SECTOR_SIZE;
	...
	if (buffer_full) {
		write_failures += write_to_flash_contiguous(flash, flash->write_buffer, W25N04KV_SECTOR_SIZE);
...
uint16_t finish_flash_write(W25N04KV_Flash *flash) {
	...
	while (flash->write_buffer_size < W25N04KV_SECTOR_SIZE)
...
void reset_flash_read_pointer(W25N04KV_Flash *flash) {
	flash->next_page_to_read = 0;
}

void read_next_2KB_from_flash(W25N04KV_Flash *flash, uint8_t *buffer) {
	read_bytes_from_page(flash, buffer,	W25N04KV_BYTES_PER_PAGE, flash->next_page_to_read, 0);
	flash->next_page_to_read++;  // Increment the page read counter

	get_ECC_status(flash);
}

uint8_t next_read_page(W25N04KV_Flash *flash) {
	return flash->next_page_to_read;
}

uint16_t erase_flash(W25N04KV_Flash *flash) {
	...
	for (uint16_t block_count = 0; block_count < W25N04KV_NUM_BLOCKS-1; block_count++) {
		erase_block(flash, block_count * W25N04KV_PAGES_PER_BLOCK);  // Address of first page in each block
...
uint32_t get_bytes_remaining(W25N04KV_Flash *flash) {
	return ((W25N04KV_NUM_BLOCKS-1) * W25N04KV_PAGES_PER_BLOCK * W25N04KV_BYTES_PER_PAGE)
			- (flash->current_page * W25N04KV_BYTES_PER_PAGE + flash->next_free_column)
			- flash->write_buffer_size;
...
uint8_t write_reserved_flash_page(W25N04KV_Flash *flash, uint8_t page_num, uint8_t* data, uint16_t data_sz) {
	// Write to the nth page of the last block of flash
	unlock_flash(flash);
	write_bytes_to_page(flash, data, data_sz,
			(W25N04KV_NUM_BLOCKS-1) * W25N04KV_PAGES_PER_BLOCK + page_num, 0);
	lock_flash(flash);

	return flash->last_write_failure_status;
}

void read_reserved_flash_page(W25N04KV_Flash *flash, uint8_t page_num, uint8_t* buffer, uint16_t buffer_sz) {
	// Grab the nth page of the last block of flash
	read_bytes_from_page(flash, buffer, buffer_sz,
			(W25N04KV_NUM_BLOCKS-1) * W25N04KV_PAGES_PER_BLOCK + page_num, 0);
}

uint8_t erase_reserved_flash_pages(W25N04KV_Flash *flash) {
	// Erase the last block only
	unlock_flash(flash);
	erase_block(flash, W25N04KV_PAGES_PER_BLOCK * (W25N04KV_NUM_BLOCKS - 1));
	lock_flash(flash);
	return flash->last_erase_failure_status;
}

uint16_t scan_bad_blocks(W25N04KV_Flash *flash, uint16_t *bad_blocks) {

	uint8_t read_byte[1];
	uint16_t num_bad_blocks = 0;

	for (uint16_t block_adr = 0; block_adr < W25N04KV_NUM_BLOCKS; block_adr++) {  // block 0, 1, ..., 1022, 1023
		read_bytes_from_page(flash, read_byte, 1, block_adr*W25N04KV_PAGES_PER_BLOCK, 0);  // page 0, 64, 128, ...
...
void add_test_delimiter(W25N04KV_Flash *flash) {
	// This is kind of dumb but it works
	uint8_t delimiter_arr[W25N04KV_BYTES_PER_PAGE] = { 0 };

	// Fill an entire page worth of bytes with 0's
	write_to_flash(flash, delimiter_arr, W25N04KV_BYTES_PER_PAGE);
}
